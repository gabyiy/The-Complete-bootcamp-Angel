...
app.post("/", function(req, res){
  ...
  // aici luam datetele ce le avem de la form le salvam si dupa le adaugam la colectia Item(input)
  const itemName=req.body.newItem 
  // iar aici salvam  ce avem la name la button aica list
 ...
 const item =new Item({
   ...
   // aici cu un if vedem daca este in default adica today sa ne monstreze pagina home altfel sa ne mostreze pagina creata
   if(listName=== "Today"){
    item.save()
    //ca sa ne apara in lista trebuie sa redirectionam catre home route
      res.redirect("/")
   }else {
     //aici specificam s gaseasca ce avem la name si in caz ca o gaseste sa o adauge la foundList
...
// aici putem acesa valuarea inputului hiden
...
Item.findByIdAndRemove(checkedItemId,function (err) {
  ...
  // aici intram in list (in primul parametru specificam ca dorim sa gasim o lista care are numele de listName),al doilea este felul in care o vom updata utilizand pull si mai exact ce anume (arrayu items)
