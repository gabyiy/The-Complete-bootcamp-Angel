...
// aici folosim iar un chain la ruta articles si facem o ruta schmbatoare



app.route("/articles/:articolTitle")


//aici vrem sa gasim un articlo in especific
.get((req,res)=>{

    // asa accesam ce avem in articolTitle

const requestedTitle= req.params.articolTitle

// iar aici vedem daca avem in baza de date acel articol
   Article.findOne({title:requestedTitle},(err,foundArticle)=>{
       if(err){
          res.send(err);
       }else{
...
// asa am caut ain postman

// http://localhost:3000/articles/bere  cu % il utilizam cand avem mai multe  cuvinte s si sew utilizeaza pentru a  le uni
// de ex http://localhost:3000/articles/bere%suc
})
// pentru a face un put utilizam put si scoatem ; de la get si le punem dupa put ca sa terminam chainu

.put((req,res)=>{
 
    

    //asa updatam tot ce punem la put in postman adica title si content
    //trebuie sa punmem si overrite true altfel nu suprascrfie
    Article.updateMany({title:req.params.articolTitle},{title:req.body.title, content:req.body.content},{overwrite:true},(err,updateArticle)=>{
     if (err){
         res.send(err)
     }else{
         res.send("succes")
...
//cand dorim sa updatam un field sspecific dintrun document trebuie sa utilizam patch
//aici cu set schimbam content cu ce introducem in postman 
.patch((req,res)=>{
Article.updateOne({title:req.params.articolTitle},{$set:{content:req.body}},(err,contentUpdated)=>{
    if (err){
        res.send(err);
    }else{
        res.send(contentUpdated);
    }
})

})
.delete((req,res)=>{
    Article.deleteOne({title:req.params.articolTitle},(err)=>{
        if(err){
           res.send(err);
        }else{
           res.send("deleted succesefuly");
        }
    })
});
